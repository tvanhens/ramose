...
**`estimate` is not filtered.** It is planner metadata derived from node counts (`packages/ramose/src/internal/core/db.ts:173-177`), never materialised datoms, so it is computed over the unfiltered db and also charges the `maxCells` budget. A 413 body and the always-emitted `x-ramose-ms` / `r2-gets` / `cache-hits` headers are therefore a bounded side channel on invisible data — accepted, not closed. `explain` is admin-only.
