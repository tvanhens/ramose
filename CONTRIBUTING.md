...
- `POST /__test__/db/:name/r2|storage|checkpoint|abort|transact|query|index|sessions` — write/corrupt real local R2, exercise storage tiers over real local R2/Cache bindings, arm/release/throw at a checkpoint, abort a DO isolate, forward `{ tx }` / `{ query | pull | entity }` to the real Transactor / Replica DO, run the real indexer, or inspect real hibernation attachments (status and body pass through; nothing invents success)
