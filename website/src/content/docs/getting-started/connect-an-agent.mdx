...
Every Ramose deployment exposes an MCP endpoint at `POST /db/<root>/mcp`. It lists `describe`, `query`, `mutate`, and `changeset`. Restricted agent credentials omit `mutate`; [changesets](/guides/changesets/) let them prepare work for human approval.
...
```text
https://your-ramose-origin.example/db/tasks/mcp
```
...
```json
{
  ...
  "url": "https://your-ramose-origin.example/db/tasks/mcp",
  "headers": {
...
The direct-operation tools are:
...
<Next links={[
  ...
  { href: "/reference/mcp/", label: "MCP reference", why: "tool contracts and errors" },
