...
Every Ramose deployment exposes one MCP endpoint at `POST /mcp`. It lists exactly three tools—`describe`, `query`, and `mutate`—regardless of how many entities, operations, or child graphs your application contains.
...
query      Execute one bounded QueryDocument against an authorized graph
mutate     Invoke one exact authorized operation with a durable receipt
```

The agent begins at its authorized root with `describe`:

```json
{
  "at": [],
  "browse": { "kinds": ["entity", "operation", "graph"] },
  "page": { "limit": 20 }
}
```
...
`mutate` does not expose raw writes. Ramose re-resolves the graph path, operation, target visibility, input schema, and grant at execution time. An identical retry with the same `invocationId` returns the original result and receipt. Reusing that id for different input returns `invocation_conflict`.
...
When the app later introduces project subgraphs, the tool list remains unchanged. Discovery may return a child path such as `at: ["acme", "launch"]`; the agent uses that path in later calls. Every segment is authorized again.
...
<Next links={[
  { href: "/guides/mcp/", label: "MCP guide", why: "graph traversal, paging, safety, and production setup" },
