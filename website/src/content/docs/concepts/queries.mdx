...
Queries return many by default. `.one()` expresses zero-or-one; `.oneOrFail()` requires exactly one. Never fetch many and choose the first when identity is part of correctness. Graph traversal uses `.one().db()` and rejects ambiguous matches.
...
Pagination is part of the query contract. Prefer stable indexed order plus cursor pagination for user-facing feeds. Aggregates operate within one graph and one authorized view; there are no cross-graph aggregates.
