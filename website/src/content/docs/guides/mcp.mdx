...
description: Expose the same graphs, queries, operations, and authorization to agents through Ramose's fixed three-tool MCP surface.
---
...
<Learn learn={["How discovery stays useful as a catalog grows", "How agents traverse child graphs without internal identifiers", "How to bound queries and make mutations safe to retry"]} before={[{ href: "/getting-started/connect-an-agent/", label: "Connect an agent" }]} minutes={14} />
...
1. Give entities, fields, graphs, and operations short descriptions that explain product meaning.
2. Model product actions as narrow operations with validated inputs.
3. Put data in child graphs when isolation, authorization, or lifecycle justifies the boundary.
4. Grant the exact reads and operations each role needs.
5. Keep common queries bounded and index-friendly.
...
{
  "at": [],
  "browse": { "kinds": ["entity", "graph", "operation"] },
  "page": { "limit": 20 }
}
```
...
## Traverse the graph hierarchy

The `at` field is a path of public graph names from the authorized root:

```json
{ "at": ["acme", "launch"] }
```

Ramose resolves and authorizes every segment. The client never receives storage ids, database ids, or graph implementation details. Use child paths returned by discovery or an authorized parent query; do not invent paths from user input.

Queries remain inside one graph. If an agent needs information from two projects, query each project graph separately and combine the plain results in the client. The same rule applies to the TypeScript API.
...
Test product behavior at the operation and query layers first. Add protocol coverage for the fixed MCP adapter: discovery pagination, catalog changes, bounded query errors, graph traversal, OAuth failures, repeated invocation ids, and inaccessible targets.
...
<Next links={[
  ...
  { href: "/best-practices/security/", label: "Security checklist", why: "review graph paths, grants, errors, and tokens" },
]} />
