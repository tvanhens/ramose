...
description: Expose the same queries, operations, and authorization to agents through Ramose's MCP surface.
---
...
Ramose's MCP server is a projection of your application model, not a second API to maintain. Deployments expose `describe`, `query`, `mutate`, and `changeset` at `POST /db/<root>/mcp`. Restricted agent credentials omit `mutate`; use [changesets](/guides/changesets/) to prepare work for human review.
