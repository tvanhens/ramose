...
| `/transact` | the only write; every key in the body is a full field name (`:todo/title`). The response is the wire body — the client reshapes it into `TxReport` |
...
| `/session` | the WebSocket connection ([glossary](/concepts/glossary/#connection)) behind `Ramose.connect`: reads travel on it, the server pushes `{ op: "t", t }` when the version moves; writes are always HTTPS |
