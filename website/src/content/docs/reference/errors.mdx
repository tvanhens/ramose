...
Every error is a tagged value and `DbError` is the union of the eight request errors. Most come from the Ramose server — the one Cloudflare Worker that serves all your databases. `.oneOrFail()` can also fail with `NotOne`, and a parameterized query can fail with `ParamError` — neither is a `DbError`. Match by name with `Effect.catchTags`.

```ts
const rows = yield* db.query(query).pipe(
  Effect.catchTags({
