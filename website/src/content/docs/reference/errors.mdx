...
`inaccessible` intentionally covers missing and unauthorized cases when distinguishing them would disclose information. `.one()` may also produce `NotOne`; catalog policy compilation may produce `PolicyError` before deployment.
