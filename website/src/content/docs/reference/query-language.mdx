...
A query is a rule: a **head** (optional declared holes) plus a **body** (its
clauses and its projection). Everyday code puts values in the fluent
chain (`Query.from(Issue).where({ done: false })`). One constructor
builds the kernel form — `Query.q(body)` — and the body has two spellings
of the same value:
...
The fluent spelling binds values at the clause — `where({ owner: me })` —
