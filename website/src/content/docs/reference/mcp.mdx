---
title: MCP server
description: Reference for Ramose's Streamable HTTP endpoint, OAuth discovery, authorized tools, paging, and errors.
---
import Next from "../../../components/Next.astro";

Ramose serves a remote Model Context Protocol endpoint at `POST /db/<root>/mcp`. It uses Streamable HTTP, OAuth protected-resource discovery, and the `describe`, `query`, `mutate`, and `changeset` tools. Restricted agent credentials omit `mutate`. See [changesets](/guides/changesets/) for proposal discovery, draft queries, revision handling, and approval.
