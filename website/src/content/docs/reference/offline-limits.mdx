...
**A pull-phase filter cannot name an entity.** `where({ author })` takes an `EntityId` or a `ClientRef` wherever a reference is, but the `where` inside a `select(…)` collection is compiled into the query value itself, before any replica binding exists. One there is refused when the query is built. Constrain the entity in the query's own `where`.
