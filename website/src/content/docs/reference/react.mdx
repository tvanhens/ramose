...
`useLive(db, query): { rows, error, ticks }` — a **live query** as React state: it re-runs whenever the database changes ([glossary](/concepts/glossary/#live-query)). Needs no provider.
...
`useQuery(db, query): { data, error, loading }` — one `db.q` per `db` / `query` pair. While a new run is loading the previous `data` stays, so a time-travel slider never flashes empty; a slower older answer never overwrites a newer one.
