...
```ts
const mine = projectDb.observe(
  projectDb.query.from(Task).where({ assignee: me.id }),
)
```

The value is entity-branded: a handle for a `Project` at `Task.assignee` is a compile error, and so is a bare number. An `EntityId` and the `ClientRef` of an entity this device created are both accepted, and a `ClientRef` keeps naming the same entity when the server issues its `EntityId` — the query follows it without being rebuilt.

An identity this replica holds nothing for — never received, or issued to a different replica — matches nothing. That is an answer, not an error: the entity is simply not visible here, and the query starts answering if it arrives.

An entity identity is only meaningful where an entity can be held. At a scalar field, in a comparison, or inside a `select(…)` collection's own `where`, one is refused when the query is built rather than quietly matching nothing.
