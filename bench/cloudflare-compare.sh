...
BENCH_LABEL="$BASE" bash bench/cloudflare.sh "$CONCURRENCY" "$SECONDS_PER_PHASE"
restore

echo ">> Benchmarking $(git rev-parse --abbrev-ref HEAD) ..."
BENCH_LABEL="$(git rev-parse --abbrev-ref HEAD)" bash bench/cloudflare.sh "$CONCURRENCY" "$SECONDS_PER_PHASE"
