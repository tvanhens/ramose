/**
 * Peer behavioral contract — install → transact → query → pull, session
 * sockets, and multi-client live. The same suite runs against Alchemy
 * local mode and a deployed `RAMOSE_URL`.
 */
...
d("ramose e2e", () => {
  let client: Peer;
  let db: ReturnType<Peer["db"]>;
  beforeAll(() => {
    client = new Peer(target.url(), {
      token: tokenOf(),
...
d("ramose session socket e2e", () => {
  ...
  test(
    ...
    async () => {
      ...
      const ramose = Ramose.connect({ url: urlOf(), token });
      try {
