...
| `src/app/ramose.ts` | the workspace wiring: `Ramose.token.jwt` over `authClient.ramose.token` plus create-time `install()` / label seeds; handed to screens as `{ slug, cls, token }` — `ensureSelf` stamps profile fields on the peer-owned `user` row |
