/**
 * Todos — the smallest end-to-end Ripple app: a catalog, a live query and
 ...
 * through VITE_RIPPLE_URL.
 *
 * Deploy: `bun alchemy deploy examples/todos/alchemy.run.ts`, then
 * `VITE_RIPPLE_URL=<peerUrl> bunx vite build examples/todos`.
 */
 ...
 * serving, and `VITE_RIPPLE_URL` points at wherever the peer actually came
...
export const Ui = Command.Dev(
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      env: { VITE_RIPPLE_URL: server.url },
    };
  }),
);
