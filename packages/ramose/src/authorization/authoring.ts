...
export type AuthRule<F extends AnyFocus = AnyFocus> = {
  readonly _tag: "AuthRule";
  readonly focus: F;
  readonly body: (ctx: RuleContext<F>) => AuthExpr;
};

export interface RuleContext<F extends AnyFocus = AnyFocus> {
  readonly me: AuthPath;
  readonly resource: AuthPath & Snapshot<F>;
  readonly claims: { readonly [key: string]: AuthPath };
  readonly input: { readonly [key: string]: AuthPath };
}
...
/** Operation authorization. Accepts a catalog Operation or `Focus.operations.name`. */
 ...
 * and `Taggable.operations.addTag`. Does not change schema identity.
 */
export function withOperations<F extends AnyFocus, const M extends Record<string, AnyOperation>>(
  ...
  for (const key of Object.keys(operations) as (keyof M)[]) {
    ...
    bound[key] = Object.assign({}, op, { [AUTH_OWNER]: focus });
  }
  return Object.assign({}, focus, { operations: bound });
}

export const ownerOfOperation = (operation: AuthOperation): AnyFocus | undefined => {
  const attached = operation[AUTH_OWNER];
  if (attached !== undefined) return attached;
  const on = operation.on;
  if (on !== undefined && isEntity(on)) return on;
  return undefined;
};

export const isTargetless = (operation: AuthOperation): boolean => ownerOfOperation(operation) === undefined;
