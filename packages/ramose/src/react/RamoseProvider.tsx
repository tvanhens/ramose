...
export const RamoseProvider = (props: RamoseProviderProps) => {
  const { url, token, fetch, webSocket, persist, policy, children } = props;
  ...
  const client = useMemo(
    () => connect({ url, token, fetch, webSocket, persist, policy }),
    [url, token, fetch, webSocket, persist, policy, generation],
  );
