...
export interface Read<A, E = DbError> {
   ...
   * again right after a live hook's inputs change. One-shot hooks keep the
   * previous value while a later run is in flight (no flash on scrub).
   * A pull of a missing record is `null`, not `undefined`.
   */
