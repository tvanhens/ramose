 ...
 *
 * The resource applies {@link import("./Server.ts").ServerAuth} onto these
 * keys when it owns the Worker. The Worker and both Durable Object classes
 * read the same type at runtime. Adding a key here is what makes it real on
 * both sides; there is no second name list to keep in sync.
 */
