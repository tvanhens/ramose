...
describe("live survives the network", () => {
  test("a dropped socket reconnects in place and the stream keeps emitting", async () => {
    ...
    peer.drop();
    await until(() => {
      const last = live.seen.at(-1) as readonly { name: string }[] | undefined;
      return peer.sockets.length >= 2 && last?.some((r) => r.name === "Bob") === true;
    });
  ...
  test("a 5xx is retried with backoff, not surfaced", async () => {
    ...
    await until(() => peer.frames.some((f) => f.op === "sync"));
