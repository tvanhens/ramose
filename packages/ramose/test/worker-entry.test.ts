 ...
 *
 * Every doc-comment and every docs page once told a standalone app to write
 ...
 * (when it is a `file://` URL) and then `realpath`, so a bare specifier
 * resolves against the working directory and does not exist. Nothing in this
 * repository noticed, because every runnable stack pointed `main` at a relative
 * path into the workspace; they all go through `import.meta.resolve` now, so
 * the spelling a reader is given is the spelling this repo runs.
 *
