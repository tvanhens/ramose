...
describe("overlay rewriteTx :db/cas", () => {
  test("rewrites a tempid subject", async () => {
    const conn = await setup(Staffs);
    const rewritten = rewritePendingTx(
      [[":db/cas", "tmp-boss", ":staff/title", "Lead", "Staff"]],
      { "tmp-boss": 2002 },
      conn.db().schema,
    );
    expect(rewritten).toEqual([[":db/cas", 2002, ":staff/title", "Lead", "Staff"]]);
  });

  test("rewrites a ref replacement tempid", async () => {
    const conn = await setup(Staffs);
    const rewritten = rewritePendingTx(
      [[":db/cas", 1001, ":staff/manager", 1002, "tmp-boss"]],
      { "tmp-boss": 2002 },
      conn.db().schema,
    );
    expect(rewritten).toEqual([[":db/cas", 1001, ":staff/manager", 1002, 2002]]);
  });

  test("rewrites a non-null ref expected tempid", async () => {
    const conn = await setup(Staffs);
    const rewritten = rewritePendingTx(
      [[":db/cas", 1001, ":staff/manager", "tmp-boss", 1002]],
      { "tmp-boss": 2002 },
      conn.db().schema,
    );
    expect(rewritten).toEqual([[":db/cas", 1001, ":staff/manager", 2002, 1002]]);
  });

  test("leaves expected: null as the absent sentinel", async () => {
    const conn = await setup(Staffs);
    const rewritten = rewritePendingTx(
      [[":db/cas", "tmp-boss", ":staff/manager", null, 1001]],
      { "tmp-boss": 2002 },
      conn.db().schema,
    );
    expect(rewritten).toEqual([[":db/cas", 2002, ":staff/manager", null, 1001]]);
  });

  test("scalar attr remaps the subject and leaves values untouched", async () => {
    const conn = await setup(Staffs);
    const rewritten = rewritePendingTx(
      [[":db/cas", "tmp-boss", ":staff/title", "tmp-boss", "Lead"]],
      { "tmp-boss": 2002 },
      conn.db().schema,
    );
    expect(rewritten).toEqual([[":db/cas", 2002, ":staff/title", "tmp-boss", "Lead"]]);
  });

  test("rewrites a nested map replacement when :db/id is a tempid", async () => {
    const conn = await setup(Staffs);
    const rewritten = rewritePendingTx(
      [[":db/cas", 1001, ":staff/manager", null, { ":db/id": "tmp-boss", ":staff/title": "Lead" }]],
      { "tmp-boss": 2002 },
      conn.db().schema,
    );
    expect(rewritten).toEqual([
      [":db/cas", 1001, ":staff/manager", null, { ":db/id": 2002, ":staff/title": "Lead" }],
    ]);
  });
});
