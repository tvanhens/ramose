...
const SCHEMA = [attribute(":k/id", "long", { ":db/unique": ":db.unique/identity" }), attribute(":k/v", "string"), attribute(":k/n", "long"), entityKind("k")];
...
describe("transactor: group commit + monotonic t", () => {
  ...
  test("concurrent clients get contiguous t; batches coalesce into few writes", async () => {
    ...
    const N = 500;
    const acks = await Promise.all(Array.from({ length: N }, (_, i) => h.transactor.transact([{ ":ramose/type": ":k", ":k/id": i, ":k/v": `v${i}` }])));
  ...
  test("a failed tx in a batch is rejected alone; the rest of the batch commits", async () => {
    ...
    const results = await Promise.allSettled([
      ...
      h.transactor.transact([{ ":ramose/type": ":k", ":k/id": 2, ":k/v": "b" }]),
    ]);
  ...
  test("storage fault: batch is all-or-nothing, instance aborts, restart continues with no gaps/dupes", async () => {
    ...
    const first = await h.transactor.transact([{ ":ramose/type": ":k", ":k/id": 1, ":k/v": "a" }]);
    // this batch (everything submitted concurrently) hits the injected failure
    const settled = await Promise.allSettled([
      h.transactor.transact([{ ":ramose/type": ":k", ":k/id": 2, ":k/v": "b" }]),
      h.transactor.transact([{ ":ramose/type": ":k", ":k/id": 3, ":k/v": "c" }]),
      h.transactor.transact([{ ":ramose/type": ":k", ":k/id": 4, ":k/v": "d" }]),
    ]);
    ...
    await expect(h.transactor.transact([{ ":ramose/type": ":k", ":k/id": 5 }])).rejects.toBeInstanceOf(TransactorDeadError);
    ...
    const again = await Promise.all([2, 3, 4].map((i) => h2.transactor.transact([{ ":ramose/type": ":k", ":k/id": i, ":k/v": "x" }])));
  ...
  test("restart replays the un-indexed log and keeps unique-identity semantics", async () => {
    const h = await fresh();
    const a = await h.transactor.transact([{ ":ramose/type": ":k", ":k/id": 7, ":k/v": "seven" }]);
    const h2 = h.restart();
    await h2.transactor.init();
    // upsert on the restarted instance resolves to the same entity
    const b = await h2.transactor.transact([{ ":ramose/type": ":k", ":k/id": 7, ":k/n": 70 }]);
    ...
    const c = await h2.transactor.transact([{ ":ramose/type": ":k", ":k/id": 8 }]);
  ...
  test("maxBatch caps the number of txs per storage write", async () => {
    ...
    await Promise.all(Array.from({ length: 95 }, (_, i) => h.transactor.transact([{ ":ramose/type": ":k", ":k/id": i }])));
...
describe("transactor: novelty subscribers", () => {
  test("subscribers get hello then one tx frame per commit, in t order", async () => {
    ...
    const acks = await Promise.all(Array.from({ length: 20 }, (_, i) => h.transactor.transact([{ ":ramose/type": ":k", ":k/id": i, ":k/v": "v" }])));
  ...
  test("late subscriber catches up from its watermark; resume works; a pruned log yields a gap frame", async () => {
    ...
    const t0 = h.transactor.t;
    await Promise.all(Array.from({ length: 10 }, (_, i) => h.transactor.transact([{ ":ramose/type": ":k", ":k/id": i }])));
...
describe("transactor: HTTP + alarm", () => {
  test("/transact, /info, /log", async () => {
    const h = await fresh();
    const r = await h.transactor.handleRequest(new Request("https://t/transact", { method: "POST", body: JSON.stringify({ tx: [{ ":ramose/type": ":k", ":k/id": 1, ":k/v": "a" }] }) }));
  ...
  test("alarm-driven indexing publishes a root, adopts it, prunes the log; reads stay consistent", async () => {
    ...
    await Promise.all(Array.from({ length: 60 }, (_, i) => h.transactor.transact([{ ":ramose/type": ":k", ":k/id": i, ":k/n": i }])));
...
describe("transactor: structured metrics", () => {
  test("commit batches, rejects, subscribers and index runs emit structured events; /info carries live metrics", async () => {
    ...
    await Promise.all(Array.from({ length: 30 }, (_, i) => h.transactor.transact([{ ":ramose/type": ":k", ":k/id": i }])));
...
describe("transactor: TxAck datoms + clientTxId", () => {
  test("ack.datoms is the wire array, not a count", async () => {
    const h = await fresh();
    const ack = await h.transactor.transact([{ ":ramose/type": ":k", ":k/id": 1, ":k/v": "a" }]);
    ...
    const http = await h.transactor.handleRequest(
      new Request("https://t/transact", { method: "POST", body: JSON.stringify({ tx: [{ ":ramose/type": ":k", ":k/id": 2, ":k/v": "b" }] }) }),
    );
  ...
  test("clientTxId replay returns the same ack and does not assign a second t", async () => {
    const h = await fresh();
    const t0 = h.transactor.t;
    const first = await h.transactor.transact([{ ":ramose/type": ":k", ":k/id": 9, ":k/v": "once" }], undefined, "c1");
    expect(first.t).toBe(t0 + 1);
    expect(first.clientTxId).toBe("c1");
    const replay = await h.transactor.transact([{ ":ramose/type": ":k", ":k/id": 9, ":k/v": "once more" }], undefined, "c1");
    expect(replay).toEqual(first);
    expect(h.transactor.t).toBe(first.t);
    const viaHttp = await h.transactor.handleRequest(
      new Request("https://t/transact", { method: "POST", body: JSON.stringify({ tx: [{ ":ramose/type": ":k", ":k/id": 10 }], clientTxId: "c1" }) }),
    );
    expect(((await viaHttp.json()) as { t: number }).t).toBe(first.t);
    expect(h.transactor.t).toBe(first.t);
    const next = await h.transactor.transact([{ ":ramose/type": ":k", ":k/id": 11 }], undefined, "c2");
